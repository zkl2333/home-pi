...
7. **`battery_keep_input` 功能我们用不了**：server 2.3.2 加了 `set_battery_keep_input` / `get battery_keep_input` 命令，对应 PiSugar 3 寄存器 BAT_CTR2 (0x21) 的 bit 7。但我们当前 PiSugar 3 固件 v1.3.4 不识别 0x21 寄存器——`set` 命令返回 `done`、i2c 直写也"成功"，但读回永远是 `0x00`（硬件静默忽略）。要解锁需升级 PiSugar 3 固件（`pisugar-programmer` + 官方固件文件，i2c 烧录有变砖风险）。当前是桌面玩具不长插，搁置即可，不要在 bootstrap 里加 set——只会写一个永远不生效的值徒增噪音。
