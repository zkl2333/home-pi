...
- 6 页内容由 `render.PAGES` 注册：概览 / 系统 / 电源 / 日历 / 天气 / 新闻。tap 行为：**单击下一页、双击上一页、长按回首页**（`current_page = 0`）。
...
8. **render.py 字体陷阱**：用了 `DejaVuSansMono.ttf`（不含中文 glyph）和 `wqy-microhei.ttc`（CJK）两套字体。**只要 mono 字体渲染的是变量值（不是 hard-coded ASCII），都要先确认这变量永远只含 ASCII**——否则中文字符显示成方框（tofu）。已踩过两次：`uptime_str`（"3天1时"）和 `bat_eta_val`（"X时YY分"）。规则：含中文用 `FONT_CJK`，纯 ASCII 才用 DejaVu Mono；新加 Snapshot 字段第一时间想"会不会含中文"。
