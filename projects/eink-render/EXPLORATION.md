...
**Context（真实因果链，勿倒因为果）**：起点不是"字宽表谬误"，是 **Overview 时钟水平/垂直都不居中**。

1. 重设计时钟（"小且单调"）时，用户先指出**竖直不居中** → 定位根因（文字 op `y=节点.top`、行盒 `fontSize×1.2`、PIL 默认 `'la'` 钉 ascender）→ **治本**：文字 op 带盒高 `h`，按字体真实 ascent/descent 在盒内居中（`'lm'`；居中元素 `textAlign:center`→`'mm'` 盒心绘）。
2. 用户再指出**水平也不居中**。我先拐了个弯：`measureText` 按 fontFamily 标定字宽表。用户一句"**字宽表很离谱，中文那么多**"否掉——**这是中途错路，不是起因**。水平真正的解是 `'mm'` 盒心绘（盒心由 flex 居中保证 == 容器中心，与估宽误差无关）。
3. 正是"字宽表为何荒谬"这一问，逼出第一性原理：**测量必须与渲染同源**。由此才看清根问题——**字体度量被劈成两个不一致的源**：布局在 JS（Yoga），测量靠 `measureText` 拍脑袋（ASCII×0.55）；真正画字在 Python（PIL/FreeType MONO，精确）。所有居中/换行/ellipsis 偏差皆源于此（浏览器无此问题，因布局引擎直接问字体、测量与光栅同一引擎）。下面三种架构调研即为消除这个二源分裂。
